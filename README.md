...
The user can also select between two different synths and three different percussion samples for each track.

In the middle of the screen we have two sets of parameters. On the left of the screen are parameters related to the composition of the piece.

On the right of the screen are parameters related to the melodic composition. A user can chose a scale by selecting the root note and scale mode.
The user can also select "Flavour Notes". Flavour notes are the notes of a scale apart from the root note. By selecting 3 and 5 for example if we are working in C major the flavour notes are E and G.
"Flavour %" selects the probabilty of one of these flavour notes being played.

Below these parameters we have three global control buttons.

- Generate creates a new midi file for playback
- Play starts the audio and visual playback
- Stop resets the audio and visual playback

Finally we have the circles to visualise the MIDI data. With the outside circles representing the first track of each set. And the inner circle shows the second track of each set which will be shifted throughout the composition.
...
![tutorial](https://github.com/hfoley03/musical-guacamole/blob/main/img/screenshot2.png?raw=?)
